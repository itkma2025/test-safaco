<?php  
    ...
    if (isset($sanitasi_post['id'])) {
        ...
        $produk  = DB::connection('safaco')->table('isi_produk_set as ips')
                    ->leftJoin('produk_satuan as ps', 'ips.id_produk', '=', 'ps.id_produk')
                    ->select(
                        'ips.id_isi_produk_set',
                        'ips.id_produk_set',
                        'ps.kode_produk',
                        'ps.nama_produk'
                    )
                ...
                <div class="text-center p-4">
                        ...
                        <div class="card-body text-start">
                            <ul class="list-group list-group-flush">
                                <li class="list-group-item">
                                    <strong>Kode produk:</strong> <?= htmlspecialchars($produk->kode_produk) ?>
                                </li>
                                <li class="list-group-item">
                                    <strong>Nama produk:</strong> <?= htmlspecialchars($produk->nama_produk) ?>
                                </li>
                            </ul>
                        </div>
                    </div>
                    <form method="POST" id="saveForm">
                        <div class="modal-body">
                            <input type="hidden" class="form-control" name="id_isi_produk_set" value="<?= $sanitasi_post['id']; ?>">
                            <input type="hidden" class="form-control" name="id_produk_set" value="<?= encryptId($produk->id_produk_set, $key_akses); ?>">
                            <input type="hidden" class="form-control" name="routes" value="produk-set">
                            <input type="hidden" class="form-control" name="action" value="<?= encryptId('delete-isi', $key_akses); ?>">
