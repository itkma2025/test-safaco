<?php  
    ...
    if (isset($sanitasi_post['id'])) {
        $id_kategori_perbaikan = decryptId($sanitasi_post['id'], $key_akses);
        $kategori_perbaikan  = DB::connection('safaco')->table('kategori_perbaikan')
                    ->select(
           ...
           ?>
                <div class="text-center p-4">
                    <h5 class="mb-4 text-danger">
                        ...
                        Apakah Anda yakin ingin menghapus data kategori perbaikan berikut?
                    </h5>
                    <div class="card border-0 shadow-lg mx-auto" style="max-width: 400px;">
                        <div class="card-body text-start">
                            <ul class="list-group list-group-flush">
                                <li class="list-group-item">
                                    <strong>Nama Kategori Perbaikan:</strong> <?= htmlspecialchars($kategori_perbaikan->nama_kategori) ?>
                                </li>
                            </ul>
                        </div>
                    </div>
                    <form method="POST" id="saveForm">
                        <div class="modal-body">
                            <input type="hidden" class="form-control" name="id_kategori_perbaikan" value="<?= $sanitasi_post['id']; ?>">
