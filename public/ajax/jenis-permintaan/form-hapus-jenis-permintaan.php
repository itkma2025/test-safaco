<?php  
    ...
    if (isset($sanitasi_post['id'])) {
        $id_jenis_permintaan = decryptId($sanitasi_post['id'], $key_akses);
        $jenis_permintaan  = DB::connection('safaco')->table('jenis_permintaan')
                    ->select(
                        'id_jenis_permintaan',
                        'nama_jenis_permintaan',
                    )
                ...
                <div class="text-center p-4">
                        ...
                        <div class="card-body text-start">
                            <ul class="list-group list-group-flush">
                                <li class="list-group-item">
                                    <strong>Nama Jenis Produksi:</strong> <?= htmlspecialchars($jenis_permintaan->nama_jenis_permintaan) ?>
                                </li>
                            </ul>
                        </div>
                    </div>
                    <form method="POST" id="saveForm">
                        <div class="modal-body">
                            <input type="hidden" class="form-control" name="id_jenis_permintaan" value="<?= $sanitasi_post['id']; ?>">
