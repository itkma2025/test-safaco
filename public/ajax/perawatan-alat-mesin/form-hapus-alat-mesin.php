<?php  
    ...
    if (isset($sanitasi_post['id'])) {
        $id_alat_mesin  = decryptId($sanitasi_post['id'], $key_akses);
        ...
        ?>
            <div class="modal-header">
                <h5 class="modal-title w-100 text-center" id="staticBackdropLabel">Konfirmasi Hapus Data Alat & Mesin</h5>
            </div>
        <?php
        if ($alat_mesin) {
           ?>
                <div class="text-center p-4">
                    <h5 class="mb-4 text-danger">
                        ...
                        Apakah Anda yakin ingin menghapus data Alat & Mesin berikut?
                    </h5>
                    <div class="card border-0 shadow-lg mx-auto" style="max-width: 400px;">
                        <div class="card-body text-start">
                            <ul class="list-group list-group-flush">
                                <li class="list-group-item">
                                    <strong>Kode alat_mesin:</strong> <?= htmlspecialchars($alat_mesin->kode_barang) ?>
                                </li>
                                <li class="list-group-item">
                                    <strong>Nama alat_mesin:</strong> <?= htmlspecialchars($alat_mesin->nama_barang) ?>
                                </li>
                            </ul>
                        </div>
                    </div>
                    <form method="POST" id="saveForm">
                        <div class="modal-body">
                            <input type="hidden" class="form-control" name="id_alat_mesin" value="<?= $sanitasi_post['id']; ?>">
