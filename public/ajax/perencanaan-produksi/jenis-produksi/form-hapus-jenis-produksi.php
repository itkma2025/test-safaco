<?php  
    ...
    if (isset($sanitasi_post['id'])) {
        $id_jenis_produksi = decryptId($sanitasi_post['id'], $key_akses);
        $jenis_produksi  = DB::connection('safaco')->table('jenis_produksi')
                    ->select(
                        'id_jenis_produksi',
                        'nama_jenis_produksi',
                    )
        ...
        ?>
            <div class="modal-header">
                <h5 class="modal-title w-100 text-center" id="staticBackdropLabel">Konfirmasi Hapus Data Jenis Produksi</h5>
            </div>
        <?php
        if ($jenis_produksi) {
           ?>
                <div class="text-center p-4">
                    <h5 class="mb-4 text-danger">
                        ...
                        Apakah Anda yakin ingin menghapus data jenis produksi berikut?
                    </h5>
                    <div class="card border-0 shadow-lg mx-auto" style="max-width: 400px;">
                        <div class="card-body text-start">
                            <ul class="list-group list-group-flush">
                                <li class="list-group-item">
                                    <strong>Nama Jenis Produksi:</strong> <?= htmlspecialchars($jenis_produksi->nama_jenis_produksi) ?>
                                </li>
                            </ul>
                        </div>
                    </div>
                    <form method="POST" id="saveForm">
                        <div class="modal-body">
                            <input type="hidden" class="form-control" name="id_jenis_produksi" value="<?= $sanitasi_post['id']; ?>">
