<?php  
   if (session_status() === PHP_SESSION_NONE) {
    ...
    if (!empty($sanitasi_post['id'])) {
        $aksi = 'edit';
        $label = 'Edit';
        $id_jenis_produksi_decrypt  = decryptId($sanitasi_post['id'], $key_akses);
        $data_jenis_produksi = DB::connection('safaco')
                            ->table('jenis_produksi')
                            ->where('id_jenis_produksi', '=', $id_jenis_produksi_decrypt)
                            ->first();
        $nama_kategori  = $data_jenis_produksi->nama_jenis_produksi;
    }

    $id_jenis_produksi = $sanitasi_post['id'] ?? "JEN_PRODUKSI" . uuid();
   
?>
<div class="modal-header">
    <h5 class="modal-title w-100 text-center" id="staticBackdropLabel"><?= $label ?> Jenis Produksi</h5>
</div>
<form method="POST" id="saveForm">
    <div class="modal-body">
        <input type="hidden" class="form-control" name="id_jenis_produksi" value="<?= $id_jenis_produksi ?>">
        <input type="hidden" class="form-control" name="routes" value="jenis-produksi">
        ...
        <!-- Nama Kategori -->
        <div class="mb-3 row">
            <label class="form-label col-md-3">Nama Jenis Produksi</label>
            <div class="col-md-9">
                <input type="text" name="nama_jenis_produksi" class="form-control" maxlength="100" oninput="filterTextOnly(this)" value="<?= $nama_kategori ?>" required>
            </div>
        </div>
    </div>
