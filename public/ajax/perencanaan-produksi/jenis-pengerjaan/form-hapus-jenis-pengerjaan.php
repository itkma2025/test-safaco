<?php  
    ...
    if (isset($sanitasi_post['id'])) {
        $id_jenis_pengerjaan = decryptId($sanitasi_post['id'], $key_akses);
        $jenis_pengerjaan  = DB::connection('safaco')->table('jenis_pengerjaan')
                    ->select(
                        'id_jenis_pengerjaan',
                        'nama_jenis_pengerjaan',
                    )
                    ->where('id_jenis_pengerjaan', $id_jenis_pengerjaan)
                    ->first();
        // Header
        ?>
            <div class="modal-header">
                <h5 class="modal-title w-100 text-center" id="staticBackdropLabel">Konfirmasi Hapus Data Jenis Pengerjaan</h5>
            </div>
        <?php
        if ($jenis_pengerjaan) {
           ?>
                <div class="text-center p-4">
                    <h5 class="mb-4 text-danger">
                        ...
                        Apakah Anda yakin ingin menghapus data jenis pengerjaan berikut?
                    </h5>
                    <div class="card border-0 shadow-lg mx-auto" style="max-width: 400px;">
                        <div class="card-body text-start">
                            <ul class="list-group list-group-flush">
                                <li class="list-group-item">
                                    <strong>Nama Jenis Pengerjaan:</strong> <?= htmlspecialchars($jenis_pengerjaan->nama_jenis_pengerjaan) ?>
                                </li>
                            </ul>
                        </div>
                    </div>
                    <form method="POST" id="saveForm">
                        <div class="modal-body">
                            <input type="hidden" class="form-control" name="id_jenis_pengerjaan" value="<?= $sanitasi_post['id']; ?>">
