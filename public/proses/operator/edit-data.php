...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // Decrypt id operator
        $id_operator = decryptId($sanitasi_post['id_operator'], $key_akses);
        ...
        foreach ($check_operator as $field => $value) {
            $query = $connSafaco->table('operator')->where($field, $value);

            // Jika ada id_produk saat edit, exclude dari pengecekan
            if (!empty($sanitasi_post['id_operator'])) {
                $query->where('id_operator', '!=', $id_operator);
            }
        ...
        $connSafaco->table('operator')->where('id_operator', $id_operator)->update($data_operator);
