...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $validator = $validatorFactory->make($sanitasi_post,
        ...
        [
            ...
            'kode_katalog'          => 'required|string|max:20',
            'nama_produk'           => 'required|string|max:100',
            'id_kategori_produk'    => 'required|string|max:50',
            'satuan_produk'         => 'required|in:pcs,set',
        ...
        [   
            ...
            // Custom error messages kode katalog
            'kode_katalog.required'  => 'Kode katalog wajib diisi.',
            'kode_katalog.max'       => 'Kode katalog maksimal 20 karakter.',
            ...
            'id_kategori_produk.max'       => 'Kategori produk maksimal 50 karakter.',
             ...
             // Custom error messages Satuan produk
            'satuan_produk.required'  => 'Satuan produk wajib diisi.',
            'satuan_produk.in'        => 'Satuan produk hanya boleh "pcs" atau "set".',
    ...
    if ($validator->fails()) {
        ...
        $logDir = __DIR__ . '/logs'; 
        $logFile = $logDir . '/edit_produk_form_' . date('Y-m-d') . '.log';
    ...
    try {
        ...
        $exists_produk_master = $connProdukMaster->table('tb_produk_satuan')
        ...
        foreach ($check_produk as $field => $value) {
            $query = $connSafaco->table('produk_satuan')->where($field, $value);

            // Jika ada id_produk saat edit, exclude dari pengecekan
            if (!empty($sanitasi_post['id_produk'])) {
        ...
        // Cek id dan kode produk
        $exists = $connSafaco->table('produk_satuan')
            ->where('nama_produk', $sanitasi_post['nama_produk'])
            ->where('id_kategori_produk', $sanitasi_post['id_kategori_produk'])
            ->whereNot('id_produk', $id_produk)
            ->exists();
        ...
        $data_produk = [
            ...
            'kode_produk'           => toNullIfEmpty($sanitasi_post['kode_produk']),
            'kode_katalog'          => toNullIfEmpty($sanitasi_post['kode_katalog']),
            'nama_produk'           => toNullIfEmpty($sanitasi_post['nama_produk']),
            'satuan_produk'         => toNullIfEmpty($sanitasi_post['satuan_produk']),
            'harga'                 => $harga,
        ...
        if (isset($_FILES['fileInput']) && $_FILES['fileInput']['error'] === UPLOAD_ERR_OK) {
            ...
            if ($file_tmp && is_uploaded_file($file_tmp)) {
                ...
                $file_path  = 'file-safaco/produk-satuan/';
                ...
                try {
                    ...
                    if (!empty($result['filename']) && !empty($result['iv']) && !empty($result['signature'])) {
                        ...
                        try {
                            ...
                            // Update data produk
                            $connSafaco->table('produk_satuan')
                                ->where('id_produk', $id_produk)
                                ->update($data_produk);

                            // Cek apakah sudah ada data gambar untuk produk ini
                            $existsGambar = $connSafaco->table('produk_gambar_satuan')
                                ->where('id_produk', $id_produk)
                                ->exists();

                            if ($existsGambar) {
                                // Jika sudah ada → update
                                include "unlink-file.php";
                                $connSafaco->table('produk_gambar_satuan')
                                    ->where('id_produk', $id_produk)
                                    ->update($data_gambar_produk);
                            } else {
                                // Jika belum ada → insert
                                $connSafaco->table('produk_gambar_satuan')->insert(array_merge($data_gambar_insert,  $data_gambar_produk));
                            }
                            ...
                            echo json_encode([
                                ...
                                'redirect_url'  => 'data-produk.php?action=produk-satuan'
                            ]);
                            exit;
                        } catch (\Exception $e) {
                            $connSafaco->rollBack();
                    ...
                    echo json_encode([
                        'status' => 'error',
                        'message' => 'Terjadi kesalahan tidak terduga saat enkripsi.' .$verify,
                        'exception' => $e->getMessage()
                    ]);  
                    exit;
                }
        ...
        // Kondisi untuk reset gambar
        $oldFilename = $sanitasi_post['dataFileName'];

        // Cek apakah data tersedia
        if(empty($oldFilename)){
            // Hapus gambar jika user reset gambar
            include "unlink-file.php";
            $connSafaco->table('produk_gambar_satuan')
                        ->where('id_produk', $id_produk)
                        ->delete();
