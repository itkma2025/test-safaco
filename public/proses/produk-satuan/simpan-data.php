...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $validator = $validatorFactory->make($sanitasi_post,
        ...
        [   
            ...
            'id_produk.max'       => 'ID produk maksimal 50 karakter.',
    ...
    try {
        ...
        foreach ($check_produk as $field => $value) {
            if ($connSafaco->table('produk_satuan')->where($field, $value)->exists()) {
                echo json_encode([
                    'status' => 'error',
                ...
                try {
                    ...
                    if (!empty($result['filename']) && !empty($result['iv']) && !empty($result['signature'])) {
                        ...
                        try {
                            ...
                            $connSafaco->table('produk_satuan')->insert($data_produk);
                           
                            // Proses simpan data gambar
                            $connSafaco->table('produk_gambar_satuan')->insert($data_gambar_produk);
