...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        foreach ($check_produk as $field => $value) {
            $query = $connSafaco->table('produk')->where($field, $value);
        ...
        // Cek id dan kode produk
        $exists = $connSafaco->table('produk')
                ...
                try {
                    ...
                    if (!empty($result['filename']) && !empty($result['iv']) && !empty($result['signature'])) {
                        ...
                        try {
                            ...
                            // Update data produk
                            $connSafaco->table('produk')
                                ->where('id_produk', $id_produk)
                                ->update($data_produk);

                            // Cek apakah sudah ada data gambar untuk produk ini
                            $existsGambar = $connSafaco->table('produk_gambar')
                                ->where('id_produk', $id_produk)
                                ->exists();
                            ...
                            if ($existsGambar) {
                                ...
                                $connSafaco->table('produk_gambar')
                                    ->where('id_produk', $id_produk)
                                    ->update($data_gambar_produk);
                            } else {
                                // Jika belum ada → insert
                                $connSafaco->table('produk_gambar')->insert(array_merge($data_gambar_insert,  $data_gambar_produk));
                            }
