...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($validator->fails()) {
        ...
        $logDir = __DIR__ . '/logs'; 
        $logFile = $logDir . '/create_isi_produk_set' . date('Y-m-d') . '.log';
    ...
    try {
        ...
        if (!$exists_data_set) {
            echo json_encode([
                'status'  => 'error',
                'message' => "Gagal menyimpan data, Data Set tidak valid"
            ]);
            exit;
        }
        ...
        // Siapkan data isi produk set
        $data_isi_set = [
            'id_isi_produk_set' => $id_isi_produk_set,
            'id_produk_set'     => $id_produk_set,
            'id_produk'         => $id_produk,
        ...
        // Proses simpan data
        $connSafaco->table('isi_produk_set')->insert($data_isi_set);
        ...
        echo json_encode([
            ...
            'redirect_url' => 'data-produk.php?action=isi-produk-set&&id=' . encryptId($id_produk_set, $key_akses)
        ]);
        exit;
