...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        foreach ($check_produk as $field => $value) {
            $query = $connSafaco->table('produk_set')->where($field, $value);

            // Jika ada id_produk saat edit, exclude dari pengecekan
            if (!empty($sanitasi_post['id_produk_set'])) {
                $query->where('id_produk_set', '!=', $id_produk_set);
            }
        ...
        $exists = $connSafaco->table('produk_set')
            ...
            ->whereNot('id_produk_set', $id_produk_set)
            ->exists();
            ...
            if ($file_tmp && is_uploaded_file($file_tmp)) {
                ...
                try {
                    ...
                    if (!empty($result['filename']) && !empty($result['iv']) && !empty($result['signature'])) {
                        ...
                        // Generate id
                        $id_gambar_produk_set = "PRD_FILE_" . uuid();

                        try {

                            $data_gambar_insert = [
                                'id_gambar_produk_set'  => $id_gambar_produk_set,
                            ...
                            if ($existsGambar) {
                                ...
                                $connSafaco->table('produk_gambar_set')
                                    ->where('id_produk_set', $id_produk_set)
                                    ->update($data_gambar_produk);
                            } else {
                                // Jika belum ada → insert
                                $connSafaco->table('produk_gambar_set')->insert(array_merge($data_gambar_insert,  $data_gambar_produk));
                            }
