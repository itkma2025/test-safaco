...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $validator = $validatorFactory->make($sanitasi_post,
        ...
        [
            ...
            'id_supplier' => [
                                'string',
                                'max:50',
                                'required_if:kondisi,Baru,Bekas',          // wajib isi kalau Baru atau Bekas
                                'prohibited_if:kondisi,Custom Sendiri (DIY)' // harus kosong kalau Custom
                            ],
        ...
        [   
            ...
            'id_merk.string'        => 'ID Merk harus berupa teks.',
            'id_merk.max'           => 'ID Merk maksimal 50 karakter.',
            'id_merk.required_if'   => 'ID Merk wajib diisi jika status merk Barang adalah Ada.',
            'id_merk.prohibited_if' => 'ID Merk harus dikosongkan jika status merk Barang adalah Tidak Ada.',
            ...
            'id_supplier.string'        => 'ID Supplier harus berupa teks.',
            'id_supplier.max'           => 'ID Supplier maksimal 50 karakter.',
            'id_supplier.required_if'   => 'ID Supplier wajib diisi jika kondisi Barang adalah Baru atau Bekas.',
            'id_supplier.prohibited_if' => 'ID Supplier harus dikosongkan jika kondisi Barang adalah Custom Sendiri (DIY).',

              // Custom error messages id lokasi
            'id_lokasi.required'  => 'ID Lokasi wajib diisi.',
            'id_lokasi.max'       => 'ID Lokasi maksimal 50 karakter.',
    ...
    try {
        ...
        // Decrypt terlebih dahulu id alat mesin
        ...
        if($sanitasi_post['status_merk'] == '1'){
            ...
            if (!$exists_merk && !$exists_supplier) {
                echo json_encode([
                    'status'  => 'error',
        ...
        $query = $connSafaco->table('alat_mesin')
            ...
            ->where(function($q) use ($check_alat_mesin) {
            ...
            if ($file_tmp && is_uploaded_file($file_tmp)) {
                ...
                try {
                    ...
                    if (!empty($result['filename']) && !empty($result['iv']) && !empty($result['signature'])) {
                        ...
                        try {
                            // Array untuk kondisi jika data gambar belum ada (Insert Data)
                            $data_gambar_insert = [
                            ...
                            // Data gambar yang ingin di simpan atau di update
                           ...
                           // Update data alat mesin
                            $connSafaco->table('alat_mesin')
                                ->where('id_alat_mesin', $id_alat_mesin)
                                ->update($data_alat_mesin);

                            // Cek apakah sudah ada data gambar untuk alat mesin ini
                            $existsGambar = $connSafaco->table('alat_mesin_gambar')
                                ->where('id_alat_mesin', $id_alat_mesin)
                                ->exists();
                             ...
                             if ($existsGambar) {
                                ...
                                $connSafaco->table('alat_mesin_gambar')
                                    ->where('id_alat_mesin', $id_alat_mesin)
                                    ->update(array_merge($data_gambar_alat_mesin, [
                                        'updated_by' => $_SESSION['id_user']
                                    ]));
                            } else {
                                // Jika belum ada → insert
                                $connSafaco->table('alat_mesin_gambar')->insert(array_merge($data_gambar_insert, $data_gambar_alat_mesin));
                            }
