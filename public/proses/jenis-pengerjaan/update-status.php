...
if ($validator->fails()) {
    ...
    $logDir = __DIR__ . '/logs'; 
    $logFile = $logDir . '/jenis_pengerjaan_form_' . date('Y-m-d') . '.log';
...
try {
    ...
    // Proses data jenis pengerjaan
    $data_jenis_pengerjaan = [
        'id_jenis_pengerjaan'   => $id_jenis_pengerjaan,
        'status_active'         => $status_active_update,
        'updated_by'            => $_SESSION['id_user']
    ];
    // Proses update status active jenis pengerjaan
    $conn->table('jenis_pengerjaan')
            ->where('id_jenis_pengerjaan', $id_jenis_pengerjaan)
            ->update($data_jenis_pengerjaan);
