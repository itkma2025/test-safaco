...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($validator->fails()) {
        ...
        $logDir = __DIR__ . '/logs'; 
        $logFile = $logDir . '/simpan_jenis_produksi_form_' . date('Y-m-d') . '.log';
    ...
    try {
        ...
        // Proses data cs
        $data_jenis_produksi = [
            'id_jenis_produksi' => $sanitasi_post['id_jenis_produksi'],
            'nama_jenis_produksi'         => $sanitasi_post['nama_jenis_produksi'],
            'created_by'            => $_SESSION['id_user']
        ];

        // Proses simpan data jenis produksi
        $conn->table('jenis_produksi')->insert($data_jenis_produksi);
