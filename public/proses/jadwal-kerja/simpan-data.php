...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $exists = $conn->table('jadwal_kerja')
            ->where([
                'nama_jam_kerja' => $sanitasi_post['nama_jam_kerja'],
                'jam_mulai' => $sanitasi_post['jam_mulai'],
        ...
        if ($exists) {
            echo json_encode([
                'status' => 'error',
                'message' => 'Jadwal kerja dengan waktu yang sama sudah ada.'
            ]);
            exit;
        }
        ...
        // Data untuk insert
        $data_jadwal = [
            'id_jadwal_kerja' => $sanitasi_post['id_jadwal_kerja'],
            'nama_jam_kerja'  => $sanitasi_post['nama_jam_kerja'],
        ...
        $conn->table('jadwal_kerja')->insert($data_jadwal);
        $conn->commit();
        ...
        echo json_encode(['status' => 'error', 'message' => 'Gagal menyimpan data jadwal kerja.']);
        exit;
    }
}
?>