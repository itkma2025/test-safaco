...
if ($validator->fails()) {
    ...
    $logDir = __DIR__ . '/logs'; 
    $logFile = $logDir . '/jenis_permintaan_form_' . date('Y-m-d') . '.log';
...
try {
    ...
    // Proses data jenis produksi
    $data_jenis_permintaan = [
        'id_jenis_permintaan'   => $id_jenis_permintaan,
        'status_active'         => $status_active_update,
        'updated_by'            => $_SESSION['id_user']
    ];
    // Proses update status active jenis produksi
