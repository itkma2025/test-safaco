...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $validator = $validatorFactory->make($sanitasi_post,
        ...
        [   
            ...
            'nama_jenis_permintaan.required'  => 'Nama jenis produksi wajib diisi.',
            'nama_jenis_permintaan.max'       => 'Nama jenis produksi maksimal 100 karakter.',
            'nama_jenis_permintaan.regex'     => 'Format nama jenis produksi hanya boleh huruf dan spasi.',
        ]
    );
    ...
    if ($validator->fails()) {
        ...
        $logDir = __DIR__ . '/logs'; 
        $logFile = $logDir . '/simpan_jenis_permintaan_form_' . date('Y-m-d') . '.log';
    ...
    try {
        ...
        if ($exists) {
            echo json_encode([
                'status'  => 'error',
                'message' => "Gagal menyimpan data, jenis produksi sudah ada"
            ]);
            exit;
        }
        ...
        $data_jenis_permintaan = [
            'id_jenis_permintaan' => $sanitasi_post['id_jenis_permintaan'],
            'nama_jenis_permintaan'         => $sanitasi_post['nama_jenis_permintaan'],
            'created_by'            => $_SESSION['id_user']
        ];

        // Proses simpan data jenis produksi
        $conn->table('jenis_permintaan')->insert($data_jenis_permintaan);
