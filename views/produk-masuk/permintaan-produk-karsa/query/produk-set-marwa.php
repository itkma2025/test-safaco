...
if (!empty($idProdukKarsa)) {
    $produkKarsaMap = $db_inventory_karsa
        ->table('tb_produk_set_marwa as tpsm')
        ->whereIn('tpsm.id_produk_master', $idProdukKarsa)
        ->select(
            'tpsm.id_set_marwa',
            'tpsm.id_produk_master',
            'tpsm.kode_set_marwa',
            'tpsm.nama_set_marwa',
            'tpsm.id_kat_produk'
        )
...
// echo "<pre>";
// print_r($produkKarsaMap->toArray());
// exit;
...
foreach ($rows as $row) {
    ...
    $id_produk_karsa = $prdKarsa->id_set_marwa ?? '-';
    $kode_produk_karsa = $prdKarsa->kode_set_marwa ?? '-';
    $nama_produk_karsa = $prdKarsa->nama_set_marwa ?? '-';
