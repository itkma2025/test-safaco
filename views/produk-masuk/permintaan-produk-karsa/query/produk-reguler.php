...
if (!empty($idProdukKarsa)) {
    $produkKarsaMap = $db_inventory_karsa
        ->table('tb_produk_reguler as tpr')
        ->whereIn('tpr.id_produk_master', $idProdukKarsa)
        ->select(
            'tpr.id_produk_reg',
            'tpr.id_produk_master',
            'tpr.kode_produk',
            'tpr.nama_produk',
            'tpr.id_kat_produk'
        )
...
foreach ($rows as $row) {
    ...
    $id_produk_karsa = $prdKarsa->id_produk_reg ?? '-';
    $kode_produk_karsa = $prdKarsa->kode_produk ?? '-';
    $nama_produk_karsa = $prdKarsa->nama_produk ?? '-';
