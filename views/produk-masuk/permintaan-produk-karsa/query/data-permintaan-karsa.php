...
$queryTotal = DB::connection('safaco')->table('permintaan_barang_karsa');
if ($search !== '') {
    $queryTotal->where('no_permintaan', 'like', "%{$search}%");
}
$total   = $queryTotal->count();
$pages   = ceil($total / $limit);

// --- Query data sesuai halaman ---
$queryData = DB::connection('safaco')->table('permintaan_barang_karsa')->orderBy('no_permintaan', 'asc');
if ($search !== '') {
    $queryData->where('no_permintaan', 'like', "%{$search}%");
}
