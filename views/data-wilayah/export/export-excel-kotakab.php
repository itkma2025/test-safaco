...
if (!file_exists($output_folder)) {
    ...
    @unlink($output_folder . "/data_kotakab_all.zip");
}
...
if ($tokenJwt) {
    ...
    try {
        $response = $client->request('GET', $domain_wilayah . 'api/data-kotakab.php', [
            'headers' => [
...
$headers = ['No', 'Kode Kota / Kabupaten', 'Nama Kota / Kabupaten', 'Nama Provinsi', 'Status'];

foreach ($data_kotakab as $i => $row) {
    $current_data[] = [
        $i + 1,
        $row['kode_kota_kab'] ?? '',
        $row['nama_kota_kab'] ?? '',
    ...
    if ($current_file_row >= $max_per_file) {
        $filename = "$output_folder/data_kotakab_$file_index.xlsx";
...
if (!empty($current_data)) {
    $filename = "$output_folder/data_kotakab_final_$file_index.xlsx";
    createSimpleXlsx($filename, $headers, $current_data);
    $all_filenames[] = $filename;
}
...
if (count($all_filenames) === 1) {
    ...
    $zip_filename = "$output_folder/data_kotakab_all.zip";
    $zip = new ZipArchive();
    if ($zip->open($zip_filename, ZipArchive::CREATE | ZipArchive::OVERWRITE) === true) {
        ...
        if (file_exists($zip_filename)) {
            ...
            header('Content-Disposition: attachment; filename="data_kotakab_all.zip"');
