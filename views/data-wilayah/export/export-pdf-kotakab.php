...
if (!empty($data_kotakab)) {
    $no = 1;
    foreach ($data_kotakab as $row) {
        $html .= '<tr>
            <td style="text-align:center;">' . $no++ . '</td>
            <td style="text-align: center;">' . htmlspecialchars($row['kode_kota_kab']) . '</td>
...
$filename = 'data_kotakab_' . date('Ymd_His') . '.pdf';
...
$mpdf->Output('data_kotakab.pdf', \Mpdf\Output\Destination::INLINE);
exit;
