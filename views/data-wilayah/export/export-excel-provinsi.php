...
if (!file_exists($output_folder)) {
    ...
    @unlink($output_folder . "/data_provinsi_all.zip");
}
...
$headers = ['No', 'Kode Provinsi', 'Nama Provinsi', 'Status'];

foreach ($data_provinsi as $i => $row) {
    $current_data[] = [
        $i + 1,
        $row['kode_provinsi'] ?? '',
        $row['nama_provinsi'] ?? '',
    ...
    if ($current_file_row >= $max_per_file) {
        $filename = "$output_folder/data_provinsi_$file_index.xlsx";
...
if (!empty($current_data)) {
    $filename = "$output_folder/data_provinsi_final_$file_index.xlsx";
    createSimpleXlsx($filename, $headers, $current_data);
    $all_filenames[] = $filename;
}
...
if (count($all_filenames) === 1) {
    ...
    $zip_filename = "$output_folder/data_provinsi_all.zip";
    $zip = new ZipArchive();
    if ($zip->open($zip_filename, ZipArchive::CREATE | ZipArchive::OVERWRITE) === true) {
        ...
        if (file_exists($zip_filename)) {
            ...
            header('Content-Disposition: attachment; filename="data_provinsi_all.zip"');
