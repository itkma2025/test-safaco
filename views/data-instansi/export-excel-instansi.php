...
$headers = ['No', 'Nama Instansi', 'Alamat', 'No. Telp', 'Email'];

foreach ($data_instansi as $i => $row) {
    $current_data[] = [
        $i + 1,
        $row['nama_instansi'] ?? '',
        $row['alamat'] ?? '',
    ...
    if ($current_file_row >= $max_per_file) {
        $filename = "$output_folder/data_instansi_$file_index.xlsx";
...
if (!empty($current_data)) {
    $filename = "$output_folder/data_instansi_final_$file_index.xlsx";
    createSimpleXlsx($filename, $headers, $current_data);
    $all_filenames[] = $filename;
}
...
if (count($all_filenames) === 1) {
    ...
    $zip_filename = "$output_folder/data_instansi_all.zip";
    $zip = new ZipArchive();
    if ($zip->open($zip_filename, ZipArchive::CREATE | ZipArchive::OVERWRITE) === true) {
        ...
        if (file_exists($zip_filename)) {
            ...
            header('Content-Disposition: attachment; filename="data_instansi_all.zip"');
