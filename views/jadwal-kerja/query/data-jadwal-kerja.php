...
$queryTotal = DB::connection('safaco')->table('jadwal_kerja');
if ($search !== '') {
    $queryTotal->where('nama_jam_kerja', 'like', "%{$search}%");
}
$total   = $queryTotal->count();
$pages   = ceil($total / $limit);

// --- Query data sesuai halaman ---
$queryData = DB::connection('safaco')->table('jadwal_kerja')->orderBy('created_date', 'asc');
if ($search !== '') {
    $queryData->where('nama_jam_kerja', 'like', "%{$search}%");
}
$data_jam_kerja = $queryData->offset($offset)->limit($limit)->get();
