...
$queryTotal = DB::connection('safaco')->table('kategori_penjualan');
if ($search !== '') {
    $queryTotal->where('kategori_penjualan', 'like', "%{$search}%");
}
$total   = $queryTotal->count();
$pages   = ceil($total / $limit);

// --- Query data sesuai halaman ---
$queryData =  DB::connection('safaco')->table('kategori_penjualan')->orderBy('kategori_penjualan', 'asc');
if ($search !== '') {
    $queryData->where('kategori_penjualan', 'like', "%{$search}%");
}
$data_kategori = $queryData->offset($offset)->limit($limit)->get();
