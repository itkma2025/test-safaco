...
$queryTotal = DB::connection('safaco')->table('produk_grade');
if ($search !== '') {
    $queryTotal->where('grade', 'like', "%{$search}%");
}
$total   = $queryTotal->count();
$pages   = ceil($total / $limit);

// --- Query data sesuai halaman ---
$queryData = DB::connection('safaco')->table('produk_grade')->orderBy('grade', 'asc');
if ($search !== '') {
    $queryData->where('grade', 'like', "%{$search}%");
}
