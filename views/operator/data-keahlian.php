<?php require_once __DIR__ . '/query/data-keahlian.php' ?>
<!-- Custom CSS -->
...
<div class="content">
            ...
            <div class="me-2 mb-2 w-custom w-md-auto">
                <div class="dropdown w-custom">
                    ...
                    <ul class="dropdown-menu dropdown-menu-end p-3">
                        <li>
                            <a href="data-operator.php?action=export-excel-keahlian" class="dropdown-item rounded-1">
    ...
    <div class="card border-0">
        <div class="card-body" style="min-height:70vh">
            <!-- Search & button -->
            <div class="d-flex flex-wrap justify-content-between align-items-center grow-2">
                <div class="text-muted medium d-flex gap-2 flex-wrap">
                    <a href="data-operator.php?action=operator" class="btn btn-outline-secondary btn-mobile">
                ...
                <div class="card w-auto mt-3 d-none">
            ...
            <div class="d-flex flex-wrap justify-content-between align-items-center">
                <div class="text-muted medium">
                    <button class="btn btn-primary btn-mobile btnKeahlian" data-bs-toggle="modal" data-bs-target="#keahlian">
                        <i class="fe fe-plus-circle me-1"></i>Tambah data keahlian
                    </button>
                </div>
                ...
                <table class="table table-striped table-bordered">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center" style="min-width: 400px;">Nama Keahlian</th>
                            <th class="text-center" style="min-width: 400px;">Alat / Mesin</th>
                    ...
                    <tbody>
                        <?php if (!$data_keahlian->isEmpty()): ?>
                            <?php $no = 1; ?>
                            <?php foreach ($data_keahlian as $row): ?>
                            <?php
                                $id_keahlian = $row->id_keahlian;
                                $status_active = $row->status_active ? 1 : 0;
                                $checked_active = $row->status_active ? 'checked' : '';
                            ?>
                                <tr>
                                    <td class="text-center"><?= $no++ ?></td>
                                    <td><?= htmlspecialchars($row->nama_keahlian) ?></td>
                                    <td><?= htmlspecialchars($row->nama_mesin ?? 'Tidak ada Alat / Mesin terpilih') ?></td>
                                    <td class="text-center">
                                        <div class="form-switch">
                                            <input class="form-check-input updateStatus" type="checkbox" value="" id="checkNativeSwitch" <?= $checked_active ?> data-id="<?= encryptId($id_keahlian, $key_akses) ?>" data-status="<?= $status_active ?>" data-action="<?= encryptId('update_status_grade', $key_akses) ?>" data-routename="keahlian" switch>
                                        </div>
                                    </td>
                                    <td class="text-center">
                                        <div>
                                    ...
                                    <td class="text-center">
                                        <button class="btn btn-sm btn-warning btnKeahlian" data-bs-toggle="modal" data-bs-target="#keahlian" data-id="<?= encryptId($id_keahlian, $key_akses) ?>">
...
<?php 
    require_once __DIR__ . '/modal-dialog/form-keahlian.php'; 
?>
