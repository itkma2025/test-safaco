...
$queryTotal = DB::connection('safaco')->table('jenis_pengerjaan');
if ($search !== '') {
    $queryTotal->where('nama_jenis_pengerjaan', 'like', "%{$search}%");
}
$total   = $queryTotal->count();
$pages   = ceil($total / $limit);

// --- Query data sesuai halaman ---
$queryData = DB::connection('safaco')->table('jenis_pengerjaan')->orderBy('nama_jenis_pengerjaan', 'asc');
if ($search !== '') {
    $queryData->where('nama_jenis_pengerjaan', 'like', "%{$search}%");
}
