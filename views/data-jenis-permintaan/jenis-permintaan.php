<?php require_once __DIR__ . '/query/data-jenis-permintaan.php' ?>
<!-- Custom CSS -->
            ...
            <div class="d-flex flex-wrap justify-content-between align-items-center">
                <div class="text-muted medium">
                    <button class="btn btn-primary btn-mobile btnForm" data-bs-toggle="modal" data-bs-target="#modalForm">
                        <i class="fe fe-plus-circle me-1"></i>Tambah data jenis permintaan
                    </button>
                </div>
                ...
                <table class="table table-striped table-bordered">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center" style="min-width: 250px;">Nama Jenis Permintaan</th>
                    ...
                    <tbody>
                        <?php if (!$data_jenis->isEmpty()) : ?>
                            ...
                            <?php
                                $id_jenis_permintaan = $row->id_jenis_permintaan;
                                $status_active = $row->status_active;
                                $checked_active = $status_active == '1' ? 'checked' : '';
                            ?>
                                <tr>
                                    <td class="align-middle text-center"><?= $no++ ?></td>
                                    <td class="align-middle text-wrap"><?= htmlspecialchars($row->nama_jenis_permintaan) ?></td>
                                    ...
                                    <td class="align-middle text-center">
                                        <div class="form-switch">
                                            <input class="form-check-input updateStatus" type="checkbox" id="checkNativeSwitch" <?= $checked_active ?> data-id="<?= encryptId($id_jenis_permintaan, $key_akses) ?>" data-status="<?= $status_active ?>" data-action="<?= encryptId('update_status_jenis', $key_akses) ?>" data-routename="jenis-permintaan" switch>
                                        </div>
                                    </td>
                                    <td class="align-middle text-center">
                                        <button class="btn btn-sm btn-warning btnForm" data-bs-toggle="modal" data-bs-target="#modalForm" data-id="<?= encryptId($id_jenis_permintaan, $key_akses) ?>">
                                            <i class="fe fe-edit"></i>
                                        </button>
                                        <?php  
                                            if($_SESSION['role'] == "Super Admin"){
                                                ?>
                                                    <button class="btn btn-sm btn-danger btnHapus" data-bs-toggle="modal" data-bs-target="#hapusData" data-id="<?= encryptId($id_jenis_permintaan, $key_akses) ?>">
...
<?php 
    require_once __DIR__ . '/modal-dialog/form-jenis-permintaan.php'; 
    require_once __DIR__ . '/modal-dialog/hapus-jenis-permintaan.php';
?>
