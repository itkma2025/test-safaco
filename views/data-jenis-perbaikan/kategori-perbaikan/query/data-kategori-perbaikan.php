...
$queryTotal = DB::connection('safaco')->table('kategori_perbaikan');
if ($search !== '') {
    $queryTotal->where('nama_kategori', 'like', "%{$search}%");
}
$total   = $queryTotal->count();
$pages   = ceil($total / $limit);

// --- Query data sesuai halaman ---
$queryData = DB::connection('safaco')->table('kategori_perbaikan')->orderBy('nama_kategori', 'asc');
if ($search !== '') {
    $queryData->where('nama_kategori', 'like', "%{$search}%");
}
$data_grade = $queryData->offset($offset)->limit($limit)->get();
