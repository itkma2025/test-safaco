<?php require_once __DIR__ . '/query/data-jenis-perbaikan.php' ?>
<!-- Custom CSS -->
...
<div class="content">
    ...
    <div class="d-md-flex d-block align-items-center justify-content-between page-breadcrumb mb-3 mt-2">
        <div class="my-auto mb-2">
            <h2 class="mb-1">Data Jenis Perbaikan</h2>
            <nav>
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item">
                        <a href="dashboard.php"><i class="ti ti-smart-home"></i></a>
                    </li>
                    <li class="breadcrumb-item" aria-current="page">Data Jenis Perbaikan</li>
                    <li class="breadcrumb-item active" aria-current="page">Jenis Perbaikan</li>
                </ol>
            </nav>
        </div>
    </div>
            ...
            <div class="d-flex flex-wrap justify-content-between align-items-center">
                <div class="text-muted medium">
                    <button class="btn btn-primary btn-mobile btnForm" data-bs-toggle="modal" data-bs-target="#modalForm">
                        <i class="fe fe-plus-circle me-1"></i>Tambah data jenis perbaikan
                    </button>
                </div>
                ...
                <table class="table table-striped table-bordered">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center" style="min-width: 250px;">Nama Jenis Perbaikan</th>
                            <th class="text-center" style="min-width: 250px;">Kategori Perbaikan</th>
                            <th class="text-center" style="min-width: 300px;">Deskripsi</th>
                        ...
                        <?php if (!$data_jenis->isEmpty()) : ?>
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td class="align-middle text-wrap"><?= htmlspecialchars($row->nama_jenis_perbaikan) ?></td>
                                    <td class="align-middle text-wrap"><?= htmlspecialchars($row->nama_kategori) ?></td>
                                    <td class="align-middle text-wrap"><?= $row->deskripsi ?></td>
                                    ...
                                    <td class="align-middle text-center">
                                        <div class="form-switch">
                                            <input class="form-check-input updateStatus" type="checkbox" value="" id="checkNativeSwitch" <?= $checked_active ?> data-id="<?= encryptId($id_jenis_perbaikan, $key_akses) ?>" data-status="<?= $status_active ?>" data-action="<?= encryptId('update_status_jenis', $key_akses) ?>" data-routename="jenis-perbaikan" switch>
                                        </div>
                                    </td>
                                    <td class="align-middle text-center">
                                        <button class="btn btn-sm btn-warning btnForm" data-bs-toggle="modal" data-bs-target="#modalForm" data-id="<?= encryptId($id_jenis_perbaikan, $key_akses) ?>">
                                            <i class="fe fe-edit"></i>
                                        </button>
                                        <?php  
                                            if($_SESSION['role'] == "Super Admin"){
                                                ?>
                                                    <button class="btn btn-sm btn-danger btnHapus" data-bs-toggle="modal" data-bs-target="#hapusData" data-id="<?= encryptId($id_jenis_perbaikan, $key_akses) ?>">
...
<?php 
    require_once __DIR__ . '/modal-dialog/form-jenis-perbaikan.php'; 
    require_once __DIR__ . '/modal-dialog/hapus-jenis-perbaikan.php';
?>
